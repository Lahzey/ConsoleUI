...
public class RenderBuffer
{
    ...
    public RenderBuffer(RenderBuffer parent, int xOffset, int yOffset, int width, int height)
    {
        ...
        ForegroundColor = parent != null ? parent.ForegroundColor : DEFAULT_FOREGROUND_COLOR;
        BackgroundColor = parent != null ? parent.BackgroundColor : DEFAULT_BACKGROUND_COLOR;
    }
    
    public RenderBuffer(int width, int height) : this(null, 0, 0, width, height)
    {
        this.characters = new char[height][];
        for (int i = 0; i < height; i++)
        {
            characters[i] = new string(' ', width).ToCharArray();
        }
        this.foregrounds = new Color[height][];
        for (int i = 0; i < height; i++)
        {
            foregrounds[i] = new Color[width];
            for (int ii = 0; ii < width; ii++) foregrounds[i][ii] = DEFAULT_FOREGROUND_COLOR;
        }
        this.backgrounds = new Color[height][];
        for (int i = 0; i < height; i++)
        {
    ...
    private void Set(int x, int y, char c, Color foregroundColor, Color backgroundColor)
    {
    ...
    public void RenderToConsole()
    {
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    sb.Append(ConsoleUIUtils.CreateColorPrefix(nextForeground, true));
                ...
                {
                    sb.Append(ConsoleUIUtils.CreateColorPrefix(nextBackground, false));
